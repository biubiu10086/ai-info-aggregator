...
def _print_usage_summary() -> None:
    # Cost estimation only works for known providers
    in_m = USAGE["input"] / 1_000_000
    out_m = USAGE["output"] / 1_000_000
    if PRICE_IN_PER_M > 0:
        cost = in_m * PRICE_IN_PER_M + out_m * PRICE_OUT_PER_M
        print(f"\n[USAGE] tokens in={USAGE['input']:,} out={USAGE['output']:,} | est. cost ${cost:.4f}")
    else:
        print(f"\n[USAGE] tokens in={USAGE['input']:,} out={USAGE['output']:,}")
...
def process_articles(articles: list[dict], api_key: str,
                     base_url: str = "https://api.deepseek.com",
                     model: str = "deepseek-v4-flash") -> tuple[list[dict], list[dict]]:
    ...
    global SCORING_MODEL, SUMMARY_MODEL
    SCORING_MODEL = model
    SUMMARY_MODEL = model
    _reset_usage()
